...
var global_transitions = [
  //array of IE transition strings
...
function flashyslideshow(setting) {
  ...
  if (contentdiv.filters) {
    //if the filters[] collection is defined on element (only in IE)
    contentdiv.style.filter = global_transitions[effectindex]; //define transition on element
    this.pause += setting.transduration; //add transition time to pause
  }
  this.filtersupport =
    contentdiv.filters && contentdiv.filters.length > 0 ? true : false; //test if element supports transitions and has one defined
  var slideshow = this;
  flashyslideshow.addEvent(
    contentdiv,
    function () {
...
(flashyslideshow.addEvent = function (target, functionref, tasktype) {
  ...
  (flashyslideshow.setopacity = function (el, degree) {
    //sets opacity of an element (FF and non IE browsers only)
    if (typeof el.style.opacity != 'undefined') el.style.opacity = degree;
    else el.style.MozOpacity = degree;
...
flashyslideshow.prototype.rotate = function () {
  var contentdiv = document.getElementById(this.wrapperid + '_inner');
  if (this.isMouseover) {
    //if mouse is over slideshow
    return;
  }
  this.currentimg =
    this.currentimg < this.imagearray.length - 1 ? this.currentimg + 1 : 0;
  ...
  if (this.filtersupport) {
    ...
    contentdiv.fadetimer = setInterval(function () {
      ...
      else clearInterval(contentdiv.fadetimer);
    }, 50); //end setInterval
  }
};
